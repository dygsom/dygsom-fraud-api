# Pricing

**Effective Date:** 2025-01-25
**Version:** 1.0.0
**Currency:** USD

Transparent pricing that scales with your business.

---

## Plans

### Startup - $99/month

- 10,000 API calls/month
- 99.5% uptime SLA
- Email support (24h response)
- 100 req/min rate limit
- Basic monitoring

**Best for:** Small businesses, MVPs

---

### Growth - $299/month

- 50,000 API calls/month
- 99.9% uptime SLA
- Priority email support (2h response)
- 500 req/min rate limit
- Advanced monitoring
- Custom fraud rules (up to 10)
- Onboarding session

**Best for:** Growing businesses, established e-commerce

**Most Popular**

---

### Enterprise - Custom Pricing

- Unlimited API calls
- 99.95% uptime SLA
- 24/7 phone support (<30min response)
- Dedicated Customer Success Manager
- Custom rate limits
- Unlimited custom fraud rules
- Custom ML model training
- White-glove onboarding

**Best for:** Large enterprises, high volume

**Contact:** sales@dygsom.com

---

## Overage Pricing

| Plan | Overage Rate |
|------|-------------|
| Startup | $0.01 per call |
| Growth | $0.008 per call |

---

## Add-Ons

- **Additional 10K calls:** $80/month (Startup)
- **Additional 50K calls:** $200/month (Growth)
- **Custom ML Model:** Starting at $5,000 (one-time)
- **Dedicated IP:** $100/month
- **Extended Data Retention (365 days):** $50/month

---

## Free Trial

**14 days** - No credit card required
- 1,000 API calls included
- Full Startup features
- No automatic billing

**Start:** https://dygsom.com/signup

---

## Volume Discounts

| Monthly Volume | Discount |
|----------------|----------|
| 100,000+ | Contact us |
| 500,000+ | Up to 15% |
| 1,000,000+ | Up to 25% |
| 5,000,000+ | Up to 40% |

---

## Billing

- **Methods:** Credit card, ACH (Enterprise), Wire (Enterprise)
- **Cycle:** Monthly or Annual (save 20%)
- **Changes:** Upgrade/downgrade anytime

---

## Plan Comparison

| Feature | Startup | Growth | Enterprise |
|---------|---------|--------|------------|
| API Calls | 10K | 50K | Unlimited |
| Uptime SLA | 99.5% | 99.9% | 99.95% |
| Support | 24h | 2h | 30min |
| Rate Limit | 100/min | 500/min | Custom |
| Phone Support | No | No | Yes |
| Custom Rules | No | 10 | Unlimited |
| Dedicated CSM | No | No | Yes |

---

## FAQ

**Can I change plans?** Yes, anytime. Prorated charges apply.

**Unused calls roll over?** No, resets monthly.

**Setup fee?** No.

**Cancel anytime?** Yes, no long-term contracts.

**Annual discount?** Yes, save 20% on annual billing.

**Non-profit discount?** Yes, contact sales@dygsom.com.

---

## Get Started

1. Sign up: https://dygsom.com/signup
2. Choose plan
3. Get API key instantly
4. Integrate in 15 minutes

**Need help?** sales@dygsom.com | https://dygsom.com/demo

---

**Version:** 1.0.0 | **Last Updated:** 2025-01-25
